...
> 从易到难六个，每个都标了建议配置与实验看点。实验任务建议在创建时给预算
> `{"max_rounds": 30}` 控成本（v2 起预算由 agent 自治，创建后改它走投信）。
> 观察呼吸过程：管理页「日志」tab（shell 输出）与 agent 自己的观察方式（由 agent
> 经邮箱告知）——事件流里每条 `llm_response` 带 `round_cost_cny` 可核算花费。
...
- 实验看点：
  - 事件流里的定时轮询节奏；`instances/<id>/workspace/uptime.csv` 数据积累
  - 制造故障（改 hosts / 断网）→ 详情抽屉「信箱」页看来信（outbox）告警
  - 投信"汇报可用率" → 看它现场统计 CSV
...
- 实验看点：
  - 事件流里 `llm_response` 的 `brain` 字段（当前用的哪家）
  - 投信让它换默认大脑（v2 起 config 归 agent 自治——热生效，进程不重启）
  - 故障转移实验：临时把 deepseek 的 key 改错（`etc/brains.toml` 改后投信把新
    配置给 agent）→ 看它粘住切换到 glm（xuseek 故障转移是"切过去就粘住"语义）
...
- **快捷观察**：`GET /api/agents/{id}/logs`（管理页详情抽屉「日志」tab）看它的
  shell 输出；`instances/<id>/workspace/BOOT.md` 是理解它"自我认知"的最佳窗口
- **唤醒实验**：休眠中在管理页「信箱」tab 投一封信，约 5 秒被轮询唤醒
- **省钱**：实验任务一律在创建时给 `max_rounds`；⑥号单次任务最便宜
